...
//			"A "+Race.GARGOYLE.getName()+" resembling a typical bipedal "+Race.HORSE_MORPH.getName()),
	...
	static {
		...
		for(int i=1; i<=9; i++) {
			try {
				...
				InputStream is = Subspecies.class.getResourceAsStream("/statusEffects/race/raceBackground.svg");
				if(is==null) {
	...
	private Subspecies(
		...
		if(iconPathName!=null) {
			try {
				InputStream is = this.getClass().getResourceAsStream("/" + iconPathName + ".svg");
				if(is==null) {
				...
				if(!iconBackgroundPathName.isEmpty()) {
					is = this.getClass().getResourceAsStream("/" + iconBackgroundPathName + ".svg");
					if(is==null) {
	...
	protected String getBipedBackground(String svg, GameCharacter character, Colour colour) {//TODO - when support other body types, add different backgrounds
		...
		if(character!=null && character.getLegConfiguration()!=LegConfiguration.BIPEDAL) {
			try {
				...
				InputStream is = this.getClass().getResourceAsStream("/statusEffects/race/raceBackgroundNonBiped.svg");
		...
		if(character!=null && (character.isTorsoBestial() || (character.isElemental() && !((Elemental)character).getSummoner().isElementalActive()))) {
			try {
				...
				InputStream is = this.getClass().getResourceAsStream("/statusEffects/race/raceBackgroundFeral.svg");
