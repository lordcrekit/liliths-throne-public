...
//			"fetish_transformation_receiving",
	...
	static {
		try {
			InputStream is = Fetish.class.getResourceAsStream("/fetishes/fetish_bimbo.svg");
			if(is==null) {
				System.err.println("Error! Fetish icon file does not exist (Trying to read from '/fetishes/fetish_bimbo.svg')!");
			}
		...
		try {
			InputStream is = Fetish.class.getResourceAsStream("/fetishes/fetish_bro.svg");
	...
	private Fetish(
		...
		if(pathName!=null) {
			try {
				InputStream is = this.getClass().getResourceAsStream("/fetishes/" + pathName + ".svg");
				if(is==null) {
