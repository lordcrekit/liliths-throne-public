...
//						return ColourListPresets.getColourListFromId("ALL_WITH_METALS");
	...
	private String getSVGImage(GameCharacter character,
		...
		if(this.equals(ClothingType.HIPS_CONDOMS)) {
			if(!colours.isEmpty()) {
				try {
					InputStream is = this.getClass().getResourceAsStream("/clothing/belt_used_condoms_base_back.svg");
					...
					if(!equippedVariant) {
						is = this.getClass().getResourceAsStream("/clothing/belt_used_condoms_base_front.svg");
							...
							if(item.getItemType().equals(ItemType.CONDOM_USED)) {
								for(int count=0; count<character.getItemCount(item); count++) {
									if(condomColours.size()<8) {
										...
										is = this.getClass().getResourceAsStream("/clothing/belt_used_condoms_"+condomColours.size()+"_back.svg");
										s += "<div style='width:100%;height:100%;position:absolute;left:0;bottom:0;padding:0;margin:0'>" + Util.inputStreamToString(is) + "</div>";
										s = getSVGWithHandledPattern(s, pattern, patternColours);
						...
						for(Colour c : condomColours) {
							is = this.getClass().getResourceAsStream("/clothing/belt_used_condoms_"+i+"_front.svg");
							s += "<div style='width:100%;height:100%;position:absolute;left:0;bottom:0;padding:0;margin:0'>" + Util.inputStreamToString(is) + "</div>";
							s = getSVGWithHandledPattern(s, pattern, patternColours);
							s = SvgUtil.colourReplacement(this.getId(),
					...
					if(!colours.isEmpty()) {
						try {
							...
							if(isMod) {
								...
								is = this.getClass().getResourceAsStream("/" + pathNameEquipped.get(slotEquippedTo) + ".svg");
					...
					if(!colours.isEmpty()) {
						try {
							...
							if(isMod) {
								...
								is = this.getClass().getResourceAsStream("/" + pathName + ".svg");
								s = Util.inputStreamToString(is);
								is.close();
							}
